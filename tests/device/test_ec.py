...
@pytest.fixture(params=[Mode.IMPORT, Mode.GENERATE])
...
@pytest.mark.parametrize("hashtype", HASHES)
@pytest.mark.parametrize("curve", ECDSA_CURVES)
def test_ecdsa_sign(info, session, keypair, curve, hashtype):
    asymkey, public_key = keypair

    data = os.urandom(64)
    if info.version < (2, 1, 0):  # Manual truncation needed
        length = min(curve.key_size // 8, hashtype.digest_size)
        resp = asymkey.sign_ecdsa(data, hash=hashtype(), length=length)
    else:
        resp = asymkey.sign_ecdsa(data, hash=hashtype())

    public_key.verify(resp, data, ec.ECDSA(hashtype()))
...
def test_bad_ecdh_keys(session):
    pubkeys = [
    ...
    for pubkey in pubkeys:
        with pytest.raises(YubiHsmDeviceError) as context:
            session.send_secure_cmd(COMMAND.DERIVE_ECDH, keyid + a2b_hex(pubkey))
        assert context.value.code == ERROR.INVALID_DATA
    key.delete()


def test_biased_k(session):
    ...
    asymkey = AsymmetricKey.put(
        session, 0, "Test ECDSA K", 0xFFFF, CAPABILITY.SIGN_ECDSA, key
    )
    ...
    for bit in bits:
        assert mincount < bit < count - mincount
...
@pytest.mark.parametrize("length", [128, 129, 2019])
def test_eddsa_sign(session, eddsa_keypair, length):
    asymkey, public_key, private_key = eddsa_keypair
    data = os.urandom(length)
    sig = asymkey.sign_eddsa(data)
    public_key.verify(sig, data)
    if private_key:  # Imported key, compare to SW signature
        assert sig == private_key.sign(data)
